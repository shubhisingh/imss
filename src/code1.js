...
input.payBeneficiaries.map((item) => {
	if(item.beneficiaryPayId && !hashTable.has(item.beneficiaryPayId)) {
		hashTable.set(item.beneficiaryPayId, item);
	}
});

function getAscendants(item) {
	let ascendants = [item.beneficiary.businessName];
	while(item.parentBenePayId) {
    item = hashTable.get(item.parentBenePayId);
		ascendants.unshift(item.beneficiary.businessName);
	}
	return ascendants;
}

let out = input.payBeneficiaries.map((item) => {
	let heirarchy = getAscendants(item);
	return {"orgHierarchy": heirarchy};
});
